...
import { PoiWasm, PoiEvent, hasActiveVirtualStick } from "../utils/poi/poiWasm";
...
const Triggaz: React.FC = () => {
  ...
  const [takeoffPos, setTakeoffPos] = useState<{ x: number; y: number } | null>(
    null
  );
  const [landingPos, setLandingPos] = useState<{ x: number; y: number } | null>(
    null
  );
  ...
  const [displayLeftHandStatus, setDisplayLeftHandStatus] =
    useState<string>("Stationary");
  const [displayRightHandStatus, setDisplayRightHandStatus] =
    useState<string>("Stationary");
  const [displayJumpStatus, setDisplayJumpStatus] =
    useState<string>("On Ground");

  // POI Module state
  const [poiTimeline, setPoiTimeline] = useState<PoiEvent[]>([]);
  const [virtualStickPath, setVirtualStickPath] = useState<string>("");
  const poiModule = useRef<PoiWasm | null>(null);
  ...
  const calculateMovement = (
    ...
    let newDirection: "up" | "down" | "left" | "right" | "stationary" =
      "stationary";
    ...
    if (
      newDirection !== handMovement.current.direction &&
      ...
      const midiNote =
        handName === "left"
          ? 64 + (handMovement.current.directionChangeCount % 12)
          : 72 + (handMovement.current.directionChangeCount % 12);
    ...
    const directionText =
      newDirection.charAt(0).toUpperCase() + newDirection.slice(1);
  ...
  const detectJump = (keypoints: poseDetection.Keypoint[]) => {
    ...
    console.log(
      `Ground: ${groundLevel.current?.toFixed(1)}, Ankle: ${avgAnkleY.toFixed(
        1
      )}, Threshold: ${dynamicJumpThreshold.toFixed(1)}, Diff: ${(
        groundLevel.current - avgAnkleY
      ).toFixed(1)}`
    );
    ...
    if (
      !isJumping.current &&
      avgAnkleY < groundLevel.current - dynamicJumpThreshold
    ) {
      ...
      updateStatusWithPersistence(
        "Jumping!",
        displayJumpStatus,
        setDisplayJumpStatus,
        jumpStatusTimer,
        1500
      );
    }
    // Detect landing - ankles return close to ground level
      ...
      isJumping.current &&
      avgAnkleY > groundLevel.current - dynamicLandingThreshold
    ) {
      ...
      if (takeoffPos) {
        ...
        const jumpInfo =
          distance > 10 ? `Landed! (${distance}px ${direction})` : "Landed!";
  ...
  const drawSkeleton = (
    ...
    const scaleFactor = Math.max(
      1,
      Math.sqrt(currentResolution / baseResolution)
    );
    ...
    BODY_PARTS.left_arm.forEach((segment) =>
      drawSegment(segment[0], segment[1], "#FF0000")
    );
    BODY_PARTS.right_arm.forEach((segment) =>
      drawSegment(segment[0], segment[1], "#00FF00")
    );
    BODY_PARTS.left_leg.forEach((segment) =>
      drawSegment(segment[0], segment[1], "#FF0000")
    );
    BODY_PARTS.right_leg.forEach((segment) =>
      drawSegment(segment[0], segment[1], "#00FF00")
    );
    BODY_PARTS.torso.forEach((segment) =>
      drawSegment(segment[0], segment[1], "#FFFF00")
    );
    BODY_PARTS.face.forEach((segment) =>
      drawSegment(segment[0], segment[1], "#0000FF")
    );
  ...
  const drawJumpMarkers = (ctx: CanvasRenderingContext2D) => {
    ...
    setJumpEvents((prev) =>
      prev.filter((event) => now - event.timestamp < fadeDuration)
    );
  };
        ...
        try {
          ...
          poses.forEach((pose) => {
            ...
            if (leftWrist && leftWrist.score && leftWrist.score > 0.5) {
              ...
              updateStatusWithPersistence(
                leftStatus,
                displayLeftHandStatus,
                setDisplayLeftHandStatus,
                leftHandStatusTimer
              );
            }
            ...
            if (rightWrist && rightWrist.score && rightWrist.score > 0.5) {
              ...
              updateStatusWithPersistence(
                rightStatus,
                displayRightHandStatus,
                setDisplayRightHandStatus,
                rightHandStatusTimer
              );
            }
            ...
            // POI Module processing (if available)
            if (poiModule.current) {
              try {
                const poiOutput = poiModule.current.processFrame(pose);

                // Update timeline with new POIs
                if (poiOutput.new_pois && poiOutput.new_pois.length > 0) {
                  setPoiTimeline((prev) =>
                    [...poiOutput.new_pois, ...prev].slice(0, 20)
                  ); // Keep last 20 events
                }

                // Update virtual stick visualization
                if (hasActiveVirtualStick(poiOutput)) {
                  setVirtualStickPath(poiOutput.virtual_stick.svg_path);
                } else {
                  setVirtualStickPath("");
                }
              } catch (error) {
                console.warn("POI processing error:", error);
              }
            }
          });
  ...
  const resetTrackingState = () => {
    ...
    if (rightHandStatusTimer.current)
      clearTimeout(rightHandStatusTimer.current);
  ...
  useEffect(() => {
    return () => {
      if (leftHandStatusTimer.current)
        clearTimeout(leftHandStatusTimer.current);
      if (rightHandStatusTimer.current)
        clearTimeout(rightHandStatusTimer.current);
      if (jumpStatusTimer.current) clearTimeout(jumpStatusTimer.current);
      if (jumpResetTimer.current) clearTimeout(jumpResetTimer.current);
    ...
    <div className="kasm-landing-container">
      ...
      >
        ...
        >
          <button
            ...
            style={{ display: "none" }}
            onClick={initMidi}
            disabled={!!midiOutput || !midiSupported}
          >
            {midiOutput
              ? "MIDI Connected"
              : midiSupported
      ...
      >
        ...
        >
          ...
          >
            <strong>Jump Status:</strong>{" "}
            <span
              style={{
                color:
                  displayJumpStatus === "Jumping!"
                    ? "#ff8888"
                    : displayJumpStatus.startsWith("Landed!")
                    ? "#88ff88"
                    : "#ffffff",
                fontWeight: "bold",
              }}
            ...
            {jumpDistance !== null && landingPos && (
              ...
              >
                Distance jumped: <strong>{jumpDistance}px</strong>
                <br />
                Landed at:{" "}
                <strong>
                  x={Math.round(landingPos.x)}, y={Math.round(landingPos.y)}
                </strong>
              </div>
            )}
          </div>
